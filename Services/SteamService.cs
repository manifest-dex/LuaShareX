...
public partial class SteamService
{
    ...
    private enum LoginMode { Auto, Credentials, Qr }

    private SteamClient? _steamClient;
    private CallbackManager? _callbackManager;
    private SteamUser? _steamUser;
    private SteamApps? _steamApps;
    private bool _callbackLoopRunning;
    ...
    private readonly string _tokenDir = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
        "LuaShareX");
    private string TokenPath => Path.Combine(_tokenDir, "token.txt");
    ...
    private void ReadLocalUsers()
    {
        ...
        try
        {
            ...
            foreach (Match user in LocalUserBlockRegex().Matches(content))
            {
                var fields = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
                foreach (Match kv in LocalUserFieldRegex().Matches(user.Groups[2].Value))
        ...
        _currentSteamId = ActiveLocalUser?.SteamId64;
    }
    ...
    private void LoadTokens()
    {
        try
        {
            ...
            if (content.StartsWith('{'))
            {
                ...
                if (root.TryGetProperty("refresh", out var r))
                    _savedRefreshToken = r.GetString();
                if (root.TryGetProperty("access", out var a))
                    _accessToken = a.GetString();
                if (root.TryGetProperty("account", out var acc))
                    _accountName = acc.GetString();
                if (root.TryGetProperty("steamid", out var sid) && sid.TryGetUInt64(out var id))
                    _steamId64 = id;
            }
            else
            {
                _savedRefreshToken = content;
            }
    ...
    private void SaveTokens(string refreshToken, string? accessToken = null, string? accountName = null, ulong steamId = 0)
    {
        try
        {
            ...
            {
                refresh = _savedRefreshToken,
                access = _accessToken,
                account = _accountName,
                steamid = _steamId64,
            });
            File.WriteAllText(TokenPath, payload);
        }
        catch { }
    }
    ...
    private async Task EnsureConnectedAsync(LoginMode mode, string? username, string? password)
    {
        ...
        // Stop the previous callback pump first: it reads the _callbackManager
        // field, so without this it would keep running and double-pump the new
        // manager alongside the new loop after the swap below.
    ...
    public Task StartSteamKitAuth(string? username = null, string? password = null)
        => EnsureConnectedAsync(LoginMode.Auto, username, password);
    ...
    private async void OnLoggedOn(SteamUser.LoggedOnCallback callback)
    {
        switch (callback.Result)
        {
            case EResult.OK:
                ...
                {
                    try { _steamId64 = _steamClient.SteamID.ConvertToUInt64(); } catch { }
            ...
            default:
                UiInvoke(() => OnError?.Invoke($"Login failed: {callback.Result}"));
                break;
        }
    }
        ...
        List<(uint appId, uint depotId)> missing)
    {
        ...
        _toast.Show("Depot keys", $"Fetching {missing.Count} key(s) from Steam...");
        int ok = 0, fail = 0;
        int done = 0;
        ...
        if (ok > 0 || fail > 0)
            _toast.Show("Depot keys", $"{ok} ready{(fail > 0 ? $", {fail} denied by Steam" : "")}.",
                error: ok == 0 && fail > 0);
        return (ok, fail);
    }
    ...
    /// <summary>
    /// Full sign-out: disconnects, deletes saved tokens and clears all
    /// session data (owned apps, names, tickets, PICS depots, fetched keys),
    /// then restores the local-file keys so local mode keeps working.
    /// </summary>
    public void Logout()
    {
        Disconnect();
        ...
        UiInvoke(() => OnStatusUpdate?.Invoke("Signed out."));
    }
